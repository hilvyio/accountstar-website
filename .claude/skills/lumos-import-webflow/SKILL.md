...
```markdown
- **Live URL** — https://www.accountstar.co.uk
- **Provenance** — hand-built Webflow site based on the Banca template
- **Code export** — unavailable; the rebuild uses the published pages as its source
- **Collection CSVs** — unavailable; the published blog currently contains no articles
- **301 redirects** — unavailable; request the CSV before domain cutover
- **Webflow site ID** — `62005eec873d7802ae23742e`
- **CMS** — static Astro data for the initial rebuild because the content is small and the live blog is empty; Astro content collections can be added when articles are ready
- **Forms** — direct email, WhatsApp and Calendly links until a hosting/form provider is confirmed
- **Search** — none; the live blog search has no content to search
- **Hosting** — undecided; the scaffold is ready for Cloudflare Workers and the Webflow site remains live until cutover
- **Breakpoints** — Lumos defaults: 30rem, 48rem and 64rem
- **Collections** — services in `src/data/site.ts`; no live CMS items to migrate
- **List filters and sorts** — none required; the live blog has no items
- **Out of scope** — pixel parity, Webflow animations/runtime, the Instagram widget, the broken course product URL, and Webflow's form backend
- **Decisions** — preserve all public routes, copy, imagery and meaningful links; simplify the design into reusable Lumos components rather than reproduce Webflow markup
- **Still open** — form provider, hosting, redirects CSV, the downloadable financial-plan file, and verified social profile URLs
```
